...
# Upgrades: PP5 removed | gnomAD SAS | Confidence levels | All input types
...
from annotator import (annotate_all, filter_rare_variants,
                       rank_variants, apply_acmg_classification)
...
st.markdown(
    ...
    "from any input to clinical report in minutes.\n\n"
    "*Ensembl VEP · ACMG/AMP 2015 + 2023 · Claude AI · gnomAD SAS*"
)
st.caption(
    "Research tool — requires review by a qualified clinical geneticist "
    "before clinical use"
)
st.divider()
...
with st.sidebar:
    ...
    age = st.text_input("Age", value="", placeholder="e.g. 34 years / 6 months")
    sex = st.selectbox(
        "Biological Sex",
    ...
    indication = st.text_area(
        ...
        placeholder="e.g. Bilateral renal cysts, hypertension, "
                    "family history of ESRD",
        height=80
    )
    clinical_features = st.text_area(
        "Clinical Features / History", value="",
        placeholder="Presenting symptoms, relevant investigations, "
                    "family history...",
        height=80
    )
    ...
    gp_correlation = st.selectbox(
        ...
        index=0,
        help="State explicitly whether the clinical features match "
    ...
    fetch_sas = st.checkbox(
        "Fetch gnomAD SAS (slower)", True,
        help="Fetches South Asian subpopulation frequency. "
...
clinical_info = parse_clinical_info({
    ...
    "gp_narrative": (
        f"Genotype-phenotype correlation: {gp_correlation}. "
        f"Clinical features: "
        f"{clinical_features[:200] if clinical_features else 'not provided.'}"
    ),
...
input_method = st.radio(
    ...
    ["VCF File", "Plain Text / Gene Name",
     "PDF Lab Report", "CSV / Excel", "SNP / rsID"],
    horizontal=True,
...
# Plain text ──────────────────────────────────────────────────────────────────
    ...
    st.info(
        "💡 Enter any variant description: gene name, HGVS notation, "
        "SNP description, or mixed text"
    )
    text_input = st.text_area(
        "Variant description",
        placeholder=(
            "Examples:\n"
            "• BRCA1 c.5266dup p.Gln1756fs Heterozygous\n"
            "• Gene: PKD1 | Variant: c.10255C>T (p.Arg3419Ter)\n"
    ...
    if st.button("🔍 Parse Variant", type="primary") and text_input.strip():
        ...
        if source_variants and not source_variants[0].get("error"):
            ...
            err = (source_variants[0].get("error", "Unknown error")
                   if source_variants else "Parse failed")
            st.error(f"❌ {err}")
            source_variants = None
...
# PDF ─────────────────────────────────────────────────────────────────────────
    ...
    st.info(
        "📄 Upload a lab report PDF — variant data will be extracted "
    ...
    if pdf_file:
        ...
        if source_variants and not source_variants[0].get("error"):
            st.success(
                f"✅ Extracted {len(source_variants)} variant(s) from PDF"
            )
            st.info(
                "📋 Review extracted variants below — edit clinical info "
                "in sidebar if needed"
            )
        else:
            err = (source_variants[0].get("error", "Extraction failed")
                   if source_variants else "Extraction failed")
            st.error(f"❌ {err}")
            source_variants = None
...
# CSV / Excel ──────────────────────────────────────────────────────────────────
    ...
    st.info(
        ...
        "Gene, HGVS_c, HGVS_p, Zygosity, Classification"
    )
    ...
    if csv_file:
        ...
        if source_variants and not source_variants[0].get("error"):
            ...
            err = (source_variants[0].get("error", "Parse failed")
                   if source_variants else "Parse failed")
            st.error(f"❌ {err}")
            source_variants = None
...
# rsID ────────────────────────────────────────────────────────────────────────
    ...
    rsid_input = st.text_input(
        ...
        help="Enter a dbSNP rsID to fetch gene, position, and ClinVar data"
    )
    if st.button("🔍 Look Up SNP", type="primary") and rsid_input.strip():
        ...
        if source_variants and not source_variants[0].get("error"):
            st.success(
                f"✅ Found: {source_variants[0].get('gene','Unknown')} "
                f"| rsID: {rsid_input}"
            )
        else:
            err = (source_variants[0].get("error", "Not found")
                   if source_variants else "Lookup failed")
            st.error(f"❌ {err}")
            source_variants = None
...
if source_variants:
    ...
    valid = [v for v in source_variants
             if not v.get("error") and isinstance(v, dict)]
    if not valid:
    ...
    try:
        ...
        needs_vep = [v for v in valid
                     if v.get("source") not in
                     ("plain_text", "csv_excel", "rsid_lookup")]
        manual    = [v for v in valid
                     if v.get("source") in
                     ("plain_text", "csv_excel", "rsid_lookup")]

        annotated_vep = annotate_all(needs_vep) if needs_vep else []
        annotated     = annotated_vep + manual

        if fetch_sas:
            progress.progress(40, "Step 2/4: Fetching gnomAD SAS frequencies...")
    ...
    path   = len([v for v in annotated
                  if v.get("acmg") in ["Pathogenic", "Likely Pathogenic"]])
    vus    = len([v for v in annotated if v.get("acmg") == "VUS"])
    benign = len([v for v in annotated
                  if v.get("acmg") in ["Likely Benign", "Benign"]])
    ...
    if acmg_counts:
        ...
        with col_info:
            ...
            st.info(
                "PP5 (external assertion) has been **removed** from this "
                "version per ACMG 2023 guidance (Biesecker & Harrison). "
    ...
    st.caption(
        ...
        "gnomAD SAS = South Asian subpopulation"
    )
    st.divider()

    # ACMG detail expanders
    if final:
        with st.expander(
            "🔬 View Full ACMG Criteria Evidence (Top Variant)",
            expanded=False
        ):
            top      = final[0]
        ...
        with st.expander(
            "📋 Evidence Panel (3billion-style)", expanded=False
        ):
    ...
    has_api_key = bool(user_api_key or os.environ.get("ANTHROPIC_API_KEY"))
    ...
    if generate_btn:
        try:
            with st.spinner("Generating clinical report..."):
                report = generate_report(
                    final, patient_id, clinical_info, report_language
                )
        except Exception as e:
        ...
        # AI disclosure — inside generate_btn block, shown only after report
        st.warning(
            "⚠️ The interpretation above was generated by Claude AI "
            "using only the computational data in this report. "
            "Gene function descriptions, disease associations, and "
            "clinical recommendations require manual verification "
            "against ClinVar, OMIM, and published literature "
            "before clinical use."
        )
        st.divider()

        # Downloads — inside generate_btn block, only after report exists
        ...
        with dl3:
            try:
                word_file = generate_word_report(
                    final, report, patient_id, report_language
                )
    ...
    # ── Quick PDF without AI (same indentation level as generate_btn) ─────────
    elif PDF_AVAILABLE:
        if st.button(
            "📕 Generate PDF Report (no AI text)", type="secondary"
        ):
            try:
                ...
                st.success(
                    "✅ PDF generated — includes ACMG table, "
                    "evidence panel, and summary box"
                )
            except Exception as e:
...
st.divider()
st.caption(
    "SpectralG v2.0 | Built using Streamlit · Ensembl VEP · "
    "Anthropic Claude | India\n\n"
    "ACMG/AMP 2015 + 2023 updates + ACMG/ACGS-2024v1.2 | "
